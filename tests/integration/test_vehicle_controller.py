...
async def test_create_vehicle_duplicate_license_plate_returns_409(client, admin_token):
    ...
    await client.post("/vehicles", json=payload, headers={"Authorization": f"Bearer {admin_token}"})
    response = await client.post("/vehicles", json=payload, headers={"Authorization": f"Bearer {admin_token}"})
